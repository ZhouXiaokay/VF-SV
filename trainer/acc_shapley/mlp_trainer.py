...
# get_selected_model_params_concat_layer, split_concat_layer_params
import sys
import time
...
class ShapleyMLPTrainer(object):
    ...
    def one_iteration(self, x, y):
        ...
        if self.is_attend == 1:
            ...
            # print("attend: ", self.bottom_model.dense.weight[0])
        # print(self.bottom_model.dense.weight[0])
    ...
    def predict(self, x):
        ...
        pos_prob = torch.softmax(pred, dim=1)[:,1].cpu().detach().numpy()
        pred = torch.softmax(pred, dim=1).max(dim=1).indices.cpu().detach().numpy()
