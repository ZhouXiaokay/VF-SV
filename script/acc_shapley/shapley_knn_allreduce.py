...
from data_loader.load_data import load_dummy_partition_with_label, load_dependent_data, load_and_split_dataset
from trainer.acc_shapley.knn_all_reduce_trainer import AllReduceTrainer
...
def run(args):
    ...
    data = all_data[rank]
    targets = all_data['labels'].astype(int)
    ...
    if args.rank == 0:
        print("test data indices: {}".format(shuffle_ind[:n_test]))
