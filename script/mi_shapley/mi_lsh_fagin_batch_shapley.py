...
def initialize_hash_label(features, tables_num: int, a: int, depth: int):
    ...
    R = np.random.normal(0,1,[depth, tables_num])
    b = np.random.normal(0, a, [1, tables_num])
...
def query(inputs, hash_tables, R, b, a, n_train):
    """
    ...
    for i, key in enumerate(hash_val):
        if key in hash_tables[i]:
            candidates.update(hash_tables[i][key])
        else:
            candidates.update([i for i in range(n_train)])
    return candidates
...
def run(args):
    ...
    for i in range(n_test):
        ...
        lsh_candidates = list(query(test_features[i], hash_tables, R, b, a, n_train))
