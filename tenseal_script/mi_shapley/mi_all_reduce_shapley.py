...
from tenseal_trainer.knn_mi.all_reduce_trainer import AllReduceTrainer
...
def run(args):
    ...
    logging.basicConfig(level=logging.DEBUG,
                        filename=code_path + '/logs/all_reduce.log',
                        datefmt='%Y/%m/%d %H:%M:%S',
    ...
    if args.rank == 0:
        log_msg = "dataset:{}".format(dataset)
        logger.info(log_msg)
    ...
    for i in range(world_size):
        ...
        n_shapley_round += 1
    if args.rank == 0:
        logger.info("calculate shapley value cost {:.2f} s".format(time.time() - shapley_start))
